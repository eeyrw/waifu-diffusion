...
class ImageStore:
    def __init__(self, data_dir: str) -> None:
        ...
        self.image_files = [os.path.join(self.data_dir, imageInfo['IMG']) for imageInfo in self.imageInfoList]
...
def main():
    ...
    weight_dtype = torch.bfloat16 if args.fp16 else torch.float32
    ...
    def save_checkpoint(global_step):
        if rank == 0:
            ...
            pipeline = StableDiffusionPipeline(
                text_encoder=text_encoder if type(text_encoder) is not torch.nn.parallel.DistributedDataParallel else text_encoder,
                vae=vae,
                unet=unet,
                tokenizer=tokenizer,
    ...
    try:
        ...
        for epoch in range(args.epochs):
            ...
            for _, batch in enumerate(train_dataloader):
                ...
                latents = torch.randn(1,4,768//8,768//8).to('cuda')
