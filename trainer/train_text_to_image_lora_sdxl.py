...
def main(args):
        ...
        if accelerator.is_main_process:
            ...
            for model in models:
                if isinstance(unwrap_model(model), type(unwrap_model(unet))):
                    unet_lora_layers_to_save = convert_state_dict_to_diffusers(get_peft_model_state_dict(unwrap_model(model)))
    ...
    def process_batch(examples):
        ...
        input_ids_one = tokenize_prompt(tokenizer_one, examples['input_texts'])
        input_ids_two = tokenize_prompt(tokenizer_two, examples['input_texts'])
        result = {
