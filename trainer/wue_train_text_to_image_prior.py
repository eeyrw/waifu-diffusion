...
def log_validation(text_encoder, tokenizer, prior, args, accelerator, weight_dtype, epoch):
    ...
    gen_pipe = WuerstchenDecoderPipeline.from_pretrained(
        "warp-ai/wuerstchen",
        torch_dtype=weight_dtype,
        cache_dir= args.model_cache_dir, local_files_only=True
    ).to(accelerator.device)
    ...
    for i in range(len(args.validation_prompts)):
        with torch.autocast("cuda"):
            ...
            prior_output = prior_pipe(args.validation_prompts[i])
            image = gen_pipe(prior_output.image_embeddings,
                            prompt=args.validation_prompts[i], return_dict=False)[0]
            # image = pipeline(
...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        pretrained_checkpoint_file = '/home/songfuqiang/StableDiffusionWeight/model_v2_stage_b.pt'
        #pretrained_checkpoint_file = hf_hub_download("dome272/wuerstchen", filename="model_v2_stage_b.pt",cache_dir=args.model_cache_dir)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                shutil.rmtree(os.path.join(output_dir, "pytorch_model"),ignore_errors=True)
