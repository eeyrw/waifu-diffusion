...
class AspectBucket:
    ...
    def get_bucket_info(self):
        return json.dumps({"buckets": self.buckets, "bucket_ratios": self._bucket_ratios_flat})

    def get_batch_iterator(self, rng=None, rank=0, num_replicas=1):
        """
        更直观&DDP无重复的版本：
        - 每桶生成独立乱序索引
        - 按 rank 切片，确保跨卡不重复
        ...
        for res, bucket_data_single in self.bucket_data.items():
            for b, idcs in bucket_data_single.items():
                ...
                if b not in buckets_merged:
                    buckets_merged.append(b)
        buckets_merged = sorted(buckets_merged, key=lambda x: (x[0], x[1]))  # (w,h) 升序

        # 2) 为每个桶构造“跨 rank 不重复”的乱序索引，并切成 batch
        all_batches = []  # 每个元素是 [(idx,w,h), ...] 一个 batch
        for b in buckets_merged:
            idcs = bucket_data_merged.get(b, [])
            # 已在 fill_buckets 截断为 batch 的整数倍
            # 桶内独立洗牌
            order = list(range(len(idcs)))
            rng.shuffle(order)
            # 跨 rank 切片，避免重复
            order = order[rank::num_replicas]
            # 切成 batch
            for i in range(0, len(order), self.batch_size):
                if i + self.batch_size <= len(order):
                    batch_indices = [idcs[j] for j in order[i:i+self.batch_size]]
                    all_batches.append([(idx, b[0], b[1]) for idx in batch_indices])
